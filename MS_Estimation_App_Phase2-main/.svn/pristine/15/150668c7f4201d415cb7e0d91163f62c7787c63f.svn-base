...
{	
	...
	public ExportExcel(IContext context, MxTemplate template, IMendixObject inputObject) throws CoreException, IOException
	{
		...
		if(excel != null && excel.getHasContents())
		{
			...
			InputStream stream = Core.getFileDocumentContent(context, excel.getMendixObject());
		...
		if(table != null)
		{
			...
			for(int i = 0; i < table.length; i++)
			{
				...
				for(int e = 0; e < table[i].length; e++)
				{
					...
					{
						columnPreset.addResultAggrValue((Double) data);
					}
	...
	public void writeData(FileDocument outputDocument) throws Exception
	{
		...
		this.book.write(out);
		Core.storeFileDocumentContent(context, outputDocument.getMendixObject(), new ByteArrayInputStream(out.toByteArray()));	
	}
